...
boru-driven command-line client **and** interactive REPL for the InvoiceExtraction
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/invoice-extraction-cli)
make build

# 2. See usage (words, entities, env vars)
./invoice-extraction-cli --help

# 3. Provide credentials once, via the environment
export INVOICE_EXTRACTION_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./invoice-extraction-cli load 1 health            # {id:1} shorthand
./invoice-extraction-cli load '{id:1}' health       # explicit match map

# 5. Override the API base URL for a single call
INVOICE_EXTRACTION_BASE=https://api.example.com ./invoice-extraction-cli load 1 health

# 6. No arguments -> interactive REPL
./invoice-extraction-cli
invoice-extraction> load 1 health
invoice-extraction> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/invoice-extraction-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/invoice-extraction-cli load 1 health
   ```
...
```sh
./invoice-extraction-cli load 1 health          # scalar shorthand for {id:1}
./invoice-extraction-cli load '{id:1}' health     # explicit match map
```
...
./invoice-extraction-cli load 1 health
```
...
Run with no arguments to open a REPL (prompt `invoice-extraction>`). Each line is
evaluated as its own boru expression:

```text
$ ./invoice-extraction-cli
invoice-extraction> load 1 health
invoice-extraction> /help
invoice-extraction> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `health`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/invoice-extraction-cli`. |
...
(`./invoice-extraction-cli <expr>`) and interactively (the REPL), and expressions compose the

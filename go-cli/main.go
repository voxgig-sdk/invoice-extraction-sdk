// Command InvoiceExtraction-cli is a CLI + REPL for the InvoiceExtraction SDK,
...
const prompt = "invoice-extraction"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "health invoice_extraction"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      INVOICE_EXTRACTION_APIKEY (api key), INVOICE_EXTRACTION_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: INVOICE_EXTRACTION_APIKEY carries the API key and
	// INVOICE_EXTRACTION_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("INVOICE_EXTRACTION_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("INVOICE_EXTRACTION_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewInvoiceExtractionSDK(opts)

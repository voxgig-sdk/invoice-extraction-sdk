...
module InvoiceExtractionUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = InvoiceExtractionContext.new({}, nil)
    end
    op = ctx.op || InvoiceExtractionOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || InvoiceExtractionResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(InvoiceExtractionError) ? err.msg : err.to_s
    ...
    sdk_err = InvoiceExtractionError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(InvoiceExtractionError)

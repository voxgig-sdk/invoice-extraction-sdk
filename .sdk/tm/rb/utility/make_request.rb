# InvoiceExtraction SDK utility: make_request
...
module InvoiceExtractionUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = InvoiceExtractionResponse.new({})
    ...
    if fetch_err
      ...
      response = InvoiceExtractionResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = InvoiceExtractionResponse.new(fetched)
    else

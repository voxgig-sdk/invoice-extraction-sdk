...
// InvoiceExtraction SDK feature test
//
...
class FtCtrl extends InvoiceExtractionControl
{
...
class FtClient
{
    ...
    private InvoiceExtractionUtility $utility;

    public function __construct(InvoiceExtractionUtility $utility, array $options)
    {
    ...
    public function get_utility(): InvoiceExtractionUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (InvoiceExtractionContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public InvoiceExtractionContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (InvoiceExtractionContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = InvoiceExtractionConfig::make_feature($name);
    ...
    private static function buildUrl(InvoiceExtractionSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof InvoiceExtractionError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new InvoiceExtractionResponse($fetched);
        }
    ...
    private function fail(InvoiceExtractionContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (InvoiceExtractionContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;

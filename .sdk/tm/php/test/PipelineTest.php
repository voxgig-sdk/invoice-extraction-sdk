...
// InvoiceExtraction SDK pipeline test
//
...
{
    private static function utility(): InvoiceExtractionUtility
    {
        return new InvoiceExtractionUtility();
    }
    ...
    private static function ctx(array $over = []): InvoiceExtractionContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new InvoiceExtractionContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new InvoiceExtractionOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof InvoiceExtractionError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new InvoiceExtractionOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = InvoiceExtractionMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new InvoiceExtractionOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = InvoiceExtractionMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new InvoiceExtractionOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = InvoiceExtractionMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new InvoiceExtractionSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = InvoiceExtractionMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new InvoiceExtractionResponse([]),
            'result' => new InvoiceExtractionResult([]),
        ]);
        [, $err] = InvoiceExtractionMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new InvoiceExtractionSpec([]),
            'response' => null,
            'result' => new InvoiceExtractionResult([]),
        ]);
        [, $err] = InvoiceExtractionMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new InvoiceExtractionSpec(['step' => 's']),
            'response' => new InvoiceExtractionResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new InvoiceExtractionResult([]),
        ]);
        [, $err] = InvoiceExtractionMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new InvoiceExtractionSpec(['step' => 's']),
            'response' => new InvoiceExtractionResponse(self::resp(200, ['v' => 1])),
            'result' => new InvoiceExtractionResult([]),
        ]);
        [, $err] = InvoiceExtractionMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new InvoiceExtractionSpec(['step' => 's']),
            'response' => new InvoiceExtractionResponse(self::resp(200, ['v' => 2])),
            'result' => new InvoiceExtractionResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        InvoiceExtractionMakeResponse::call($ctx);
    ...
    {
        $preset = new InvoiceExtractionResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new InvoiceExtractionSpec([]),
            'response' => new InvoiceExtractionResponse([]),
            'result' => new InvoiceExtractionResult([]),
        ...
        [$got, $err] = InvoiceExtractionMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new InvoiceExtractionResult([])]);
        [, $err] = InvoiceExtractionMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new InvoiceExtractionSpec([]), 'result' => null]);
        [, $err] = InvoiceExtractionMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new InvoiceExtractionSpec(['step' => 's']),
            'result' => new InvoiceExtractionResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new InvoiceExtractionOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = InvoiceExtractionMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new InvoiceExtractionSpec(['step' => 's']),
            'result' => new InvoiceExtractionResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new InvoiceExtractionOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = InvoiceExtractionMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new InvoiceExtractionResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new InvoiceExtractionSpec([]),
            'result' => new InvoiceExtractionResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = InvoiceExtractionMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = InvoiceExtractionMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new InvoiceExtractionError('boom', 'boom');
        $utility->fetcher = function (InvoiceExtractionContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new InvoiceExtractionSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = InvoiceExtractionMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (InvoiceExtractionContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = InvoiceExtractionMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(InvoiceExtractionResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        InvoiceExtractionMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (InvoiceExtractionContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new InvoiceExtractionSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = InvoiceExtractionMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new InvoiceExtractionResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new InvoiceExtractionSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = InvoiceExtractionMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = InvoiceExtractionMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = InvoiceExtractionMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new InvoiceExtractionResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, InvoiceExtractionDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new InvoiceExtractionResult(['ok' => false])]);
        $this->expectException(InvoiceExtractionError::class);
        InvoiceExtractionDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new InvoiceExtractionResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', InvoiceExtractionMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        InvoiceExtractionMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new InvoiceExtractionBaseFeature();
        $b = new InvoiceExtractionBaseFeature();
        InvoiceExtractionFeatureAdd::call($ctx, $a);
        InvoiceExtractionFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): InvoiceExtractionBaseFeature
    {
        $f = new InvoiceExtractionBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        InvoiceExtractionFeatureAdd::call($ctx, self::named_feature('a'));
        InvoiceExtractionFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        InvoiceExtractionFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        InvoiceExtractionFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        InvoiceExtractionFeatureAdd::call($ctx, $replace);
        ...
        InvoiceExtractionFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): InvoiceExtractionContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new InvoiceExtractionSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = InvoiceExtractionPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        InvoiceExtractionPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new InvoiceExtractionResponse(['headers' => null]),
            'result' => new InvoiceExtractionResult([]),
        ]);
        InvoiceExtractionResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        InvoiceExtractionResultBody::call($ctx);

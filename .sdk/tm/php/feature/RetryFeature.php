...
// InvoiceExtraction SDK retry feature
...
class InvoiceExtractionRetryFeature extends InvoiceExtractionBaseFeature
{
    ...
    public function init(InvoiceExtractionContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (InvoiceExtractionContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(InvoiceExtractionContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(InvoiceExtractionContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;

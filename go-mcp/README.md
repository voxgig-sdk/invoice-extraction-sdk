# invoice-extraction-mcp

[MCP](https://modelcontextprotocol.io) server exposing the InvoiceExtraction SDK as
two agent tools — `invoice-extraction_list` and `invoice-extraction_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/invoice-extraction-mcp)
make build

# 2. Provide credentials via the environment
export INVOICE_EXTRACTION_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user invoice-extraction \
  -- /absolute/path/to/invoice-extraction-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./invoice-extraction-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// invoice-extraction_list: first page of records
{ "entity": "health" }
{ "entity": "health", "query": { } }

// invoice-extraction_load: one record by id
{ "entity": "health", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/invoice-extraction-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user invoice-extraction \
     -- "$PWD"/dist/*/invoice-extraction-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `invoice-extraction_list` and `invoice-extraction_load` tools now appear
   in new sessions. Ask the agent to *"list health using invoice-extraction"*
   and it calls `invoice-extraction_list` with `{"entity":"health"}`.
...
```sh
export INVOICE_EXTRACTION_APIKEY=sk_live_xxx            # API key
export INVOICE_EXTRACTION_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `invoice-extraction_list` tool
...
### Call the `invoice-extraction_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `invoice-extraction_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `invoice-extraction_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `INVOICE_EXTRACTION_APIKEY` | API key sent with every request. |
| `INVOICE_EXTRACTION_BASE` | Optional override of the API base URL. |

### Entities

The 2 entities valid as the `entity` argument:

health | invoice_extraction

### Smoke test via HTTP (raw JSON-RPC)

```sh
./invoice-extraction-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"invoice-extraction_load","arguments":{"entity":"health","query":{"id":1}}}}'
```

...
// InvoiceExtraction SDK InvoiceExtraction entity
...
class InvoiceExtractionEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new InvoiceExtractionEntity($this->_client, $opts);
    }

    /**
     * @param InvoiceExtraction|array $args InvoiceExtraction data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = InvoiceExtractionHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return InvoiceExtraction|array The current InvoiceExtraction data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of InvoiceExtraction fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = InvoiceExtractionHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of InvoiceExtraction fields).
     */
    ...
    /**
     * Create a new InvoiceExtraction.
     *
     * @param InvoiceExtractionCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed InvoiceExtractionCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return InvoiceExtraction|array The created InvoiceExtraction as an assoc-array at the
     *   SDK boundary; throws InvoiceExtractionError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                if ($ctx->result->resdata) {
                    $this->_data = InvoiceExtractionHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }

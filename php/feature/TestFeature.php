...
// InvoiceExtraction SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class InvoiceExtractionTestFeature extends InvoiceExtractionBaseFeature
{
    ...
    public function init(InvoiceExtractionContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (InvoiceExtractionContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = InvoiceExtractionParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (InvoiceExtractionContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(InvoiceExtractionContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = InvoiceExtractionParam::call($ctx, $k);

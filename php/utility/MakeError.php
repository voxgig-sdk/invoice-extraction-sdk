...
class InvoiceExtractionMakeError
{
    public static function call(?InvoiceExtractionContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new InvoiceExtractionContext([], null);
        }
        $op = $ctx->op ?? new InvoiceExtractionOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new InvoiceExtractionResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof InvoiceExtractionError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new InvoiceExtractionError('', $msg, $ctx);

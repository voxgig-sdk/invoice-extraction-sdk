...
require_relative 'InvoiceExtraction_types'
...
class InvoiceExtractionSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = InvoiceExtractionHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = InvoiceExtractionHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, InvoiceExtractionFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    InvoiceExtractionUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = InvoiceExtractionHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = InvoiceExtractionHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = InvoiceExtractionHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
    ...
    ctx.spec = InvoiceExtractionSpec.new({
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => InvoiceExtractionError.new(
        "#{op}_allow",
  ...
  def raw_request(fetchargs = {})
    ...
    rescue InvoiceExtractionError => err
    ...
    if fetched.is_a?(Hash)
      status = InvoiceExtractionHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = InvoiceExtractionError.new(
        "graphql_error", "InvoiceExtractionSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  # Canonical facade: client.InvoiceExtraction.list / client.InvoiceExtraction.load({ "id" => ... })
  def InvoiceExtraction(data = nil)
    require_relative 'entity/invoice_extraction_entity'
    InvoiceExtractionEntity.new(self, data)
  end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = InvoiceExtractionSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end

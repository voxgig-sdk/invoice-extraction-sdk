...
class InvoiceExtractionEntity
  ...
  def make
    opts = @_entopts.dup
    InvoiceExtractionEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = InvoiceExtractionHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [InvoiceExtraction, Hash] the current InvoiceExtraction data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = InvoiceExtractionHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of InvoiceExtraction fields)
  def match_get
  ...
  #
  # @param reqdata [InvoiceExtractionCreateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [InvoiceExtraction, Hash] the created InvoiceExtraction; raises InvoiceExtractionError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        if ctx.result.resdata
          @_data = InvoiceExtractionHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end

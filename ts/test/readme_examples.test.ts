...
const SDK_NAME = 'InvoiceExtractionSDK'
...
const TEST_SEED = {"entity":{"health":{"test01":{"id":"test01"}},"invoice_extraction":{"test01":{"id":"test01"}}}}
...
function isRunnable(code: string): boolean {
  return (
    /new\s+InvoiceExtractionSDK\b/.test(code) ||
    /\bInvoiceExtractionSDK\.test\b/.test(code) ||
    /\bclient\s*\./.test(code)
  )
}
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+InvoiceExtractionSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/InvoiceExtractionSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)

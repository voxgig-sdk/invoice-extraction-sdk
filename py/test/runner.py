# InvoiceExtraction SDK test runner
...
class InvoiceExtractionTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            InvoiceExtractionTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = InvoiceExtractionTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = InvoiceExtractionTestRunner.getenv("INVOICE_EXTRACTION_TEST_LIVE")
        override = InvoiceExtractionTestRunner.getenv("INVOICE_EXTRACTION_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = InvoiceExtractionTestRunner.getenv(key)
        ...
        explain = InvoiceExtractionTestRunner.getenv("INVOICE_EXTRACTION_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["INVOICE_EXTRACTION_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if InvoiceExtractionTestRunner._test_control is not None:
            return InvoiceExtractionTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                InvoiceExtractionTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            InvoiceExtractionTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = InvoiceExtractionTestRunner.load_test_control()
...
def load_env_local():
    InvoiceExtractionTestRunner.load_env_local()


def env_override(m):
    return InvoiceExtractionTestRunner.env_override(m)


def entity_data(v):
    return InvoiceExtractionTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return InvoiceExtractionTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return InvoiceExtractionTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return InvoiceExtractionTestRunner.load_test_control()


def live_delay_ms():
    return InvoiceExtractionTestRunner.live_delay_ms()

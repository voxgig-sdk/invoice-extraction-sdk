...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("INVOICEEXTRACTION_TEST_LIVE") == "TRUE" ||
		os.Getenv("INVOICEEXTRACTION_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("INVOICEEXTRACTION_TEST_EXPLAIN"); explain != "" {

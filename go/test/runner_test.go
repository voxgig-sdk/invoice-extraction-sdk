...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("INVOICE_EXTRACTION_TEST_LIVE") == "TRUE" ||
		os.Getenv("INVOICE_EXTRACTION_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("INVOICE_EXTRACTION_TEST_EXPLAIN"); explain != "" {
...
type entityTestSetup struct {
	client         *sdk.InvoiceExtractionSDK
	data           map[string]any
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.InvoiceExtractionSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.InvoiceExtractionError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.InvoiceExtractionSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.InvoiceExtractionError{Msg: msg, Code: code}
}

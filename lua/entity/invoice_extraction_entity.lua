...
local InvoiceExtractionEntity = {}
InvoiceExtractionEntity.__index = InvoiceExtractionEntity


function InvoiceExtractionEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, InvoiceExtractionEntity)
  self._name = "invoice_extraction"
  self._client = client
...
function InvoiceExtractionEntity:get_name()
  return self._name
end


function InvoiceExtractionEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return InvoiceExtractionEntity.new(self._client, opts)
end
...
function InvoiceExtractionEntity:mark_deleted()
  self._deleted = true
end


function InvoiceExtractionEntity:deleted()
  return true == self._deleted
end


function InvoiceExtractionEntity:data_set(args)
  if args ~= nil then
...
function InvoiceExtractionEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function InvoiceExtractionEntity:match_set(args)
  if args ~= nil then
...
function InvoiceExtractionEntity:match_get()
...
function InvoiceExtractionEntity:stream(action, args, callopts)
...
---@param reqdata InvoiceExtractionCreateData
---@param ctrl? table
---@return InvoiceExtraction
---@return string? err
function InvoiceExtractionEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function InvoiceExtractionEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return InvoiceExtractionEntity
